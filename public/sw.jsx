...
function Logo({size=32}){
  return <svg width={size} height={size} viewBox="0 0 32 32" fill="none"><rect width="32" height="32" rx="8" fill={C.olive}/><path d="M7 21L11 15L15.5 18L20.5 11L25 15.5" stroke={C.gold} strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"/><circle cx="25" cy="10.5" r="2.8" fill={C.gold}/><line x1="9" y1="25" x2="23" y2="25" stroke={C.gold} strokeWidth="1.2" strokeLinecap="round" opacity="0.5"/></svg>;
}
...
function InstallBanner({onInstall,onDismiss}){
  return(
    <div style={{background:C.olive,margin:"12px 16px 0",borderRadius:16,padding:"14px 16px",display:"flex",alignItems:"center",gap:12}}>
      <Logo size={36}/>
      <div style={{flex:1}}>
...
export default function App(){
  ...
  // ── FETCH listings ─────────────────────────────────────────────────────────
  // KEY FIX: filter by camp on the DB side, not client side
  // Also added .limit(50) for speed
  const fetchListings = useCallback(async(campVal)=>{
    const c=campVal||camp;
    if(!c)return;
  ...
  // ── REALTIME subscription — fix: subscribe once, resubscribe on camp change ─
  useEffect(()=>{
    if(!camp)return;
    // expire stale (fire and forget)
    supabase.from("listings").update({status:"done"}).eq("status","pending")
      .lt("requested_at",new Date(Date.now()-86400000).toISOString());
    fetchListings();
    // Realtime — listen to ALL listing changes for this camp
    const ch=supabase.channel(`listings:${camp}`)
      .on("postgres_changes",{event:"INSERT",schema:"public",table:"listings",filter:`camp=eq.${camp}`},
        ()=>fetchListings())
      .on("postgres_changes",{event:"UPDATE",schema:"public",table:"listings",filter:`camp=eq.${camp}`},
        ()=>fetchListings())
  ...
  async function handleSignup(){
    setLoading(true);
    try{
      const{data:existing}=await supabase.from("users").select("id,name,camp,platoon").eq("phone",phone).single();
      if(existing){
  ...
  async function saveProfileEdits(){
    ...
    try{
      ...
      if(!Object.keys(updates).length){setPView("settings");return;}
      ...
      setPView("settings");
      // re-fetch listings for new camp if changed
  ...
  async function requestNotifications(){
    ...
    if(perm==="granted"){
      new Notification("SwapKit 🎉",{body:"You'll now get notified when someone requests your swap!",icon:"/icons/icon-192.png"});
    }
  }
  ...
  async function postListing(){
    ...
    try{
      ...
      setListings(prev=>[{id:listing.id,status:"available",img:imgUrl,camp,platoon:postPlat||platoon,
      ...
      setTimeout(()=>fetchListings(),1500); // sync after brief delay
    }finally{setLoading(false);}
  }
  ...
  const myListings=listings.filter(l=>l.userId===uid||l.name===userName);
        ...
        <div style={WRAP}>
          ...
          <div style={{flex:1,overflowY:"auto",paddingBottom:80}}>
            <div style={{background:C.olive,padding:"36px 24px 32px",textAlign:"center",position:"relative",overflow:"hidden"}}>
              ...
              <div style={{fontSize:12,color:C.white+"77",fontFamily:F.b}}>Version 1.0 · NYSC Camp Edition</div>
            </div>
            <div style={{padding:"20px 18px"}}>
              ...
              <div style={{background:C.white,border:`0.5px solid ${C.border}`,borderRadius:18,padding:18,marginBottom:12}}>
                ...
                {[["📋","Post your listing","Tell us what you were given and what size you need."],["🔍","Browse the feed","Scroll listings from your camp only."],["📞","Request a swap","Tap request — contact revealed immediately."]].map(([em,t,d],i)=>(
                  <div key={i} style={{display:"flex",gap:13,marginBottom:i<2?14:0,paddingBottom:i<2?14:0,borderBottom:i<2?`0.5px solid ${C.border}`:"none"}}>
        ...
        <div style={WRAP}>
          ...
          <div style={{flex:1,overflowY:"auto",paddingBottom:80}}>
            ...
            <div style={{background:C.white,border:`0.5px solid ${C.border}`,borderRadius:16,margin:"8px 16px 14px",overflow:"hidden"}}>
              <SettingRow emoji="✏️" label="Edit profile" sub="Name, camp, platoon" onClick={()=>{setEditName(userName);setEditPlatoon(platoon);setEditCamp(camp);setPView("editprofile");}}/>
          ...
          <div style={{flex:1,overflowY:"auto",paddingBottom:80}}>
            <div style={{background:C.olive,margin:16,borderRadius:20,padding:"24px 20px",position:"relative",overflow:"hidden"}}>
              ...
              <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:16}}>
                ...
                <button onClick={()=>{setEditName(userName);setEditPlatoon(platoon);setEditCamp(camp);setPView("editprofile");}} style={{background:C.white+"22",border:"none",borderRadius:10,padding:"7px 12px",color:C.white,fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:F.b}}>Edit</button>
              </div>
            ...
            <div style={{margin:"12px 16px 0",fontSize:11,color:C.inkLight,textAlign:"center",fontFamily:F.b,lineHeight:1.6}}>SwapKit v1.0 · Made for Nigerian corpers 🇳🇬<br/>subeyeapp@gmail.com</div>
          </div>
