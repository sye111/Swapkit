...
// Inline SVG logo — works everywhere including PWA, no image file needed
function Logo({size=32}){
  return(
    <svg width={size} height={size} viewBox="0 0 32 32" fill="none" xmlns="http://www.w3.org/2000/svg">
      <rect width="32" height="32" rx="8" fill="#2D3A1F"/>
      <path d="M7 21L11 15L15.5 18L20.5 11L25 15.5" stroke="#F0B429" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"/>
      <circle cx="25" cy="10.5" r="2.8" fill="#F0B429"/>
      <line x1="9" y1="25" x2="23" y2="25" stroke="#F0B429" strokeWidth="1.2" strokeLinecap="round" opacity="0.5"/>
    </svg>
  );
}
...
function Toast({msg}){
  if(!msg)return null;
  return <div style={{position:"fixed",top:16,left:"50%",transform:"translateX(-50%)",zIndex:9999,background:C.olive,color:C.white,borderRadius:14,padding:"12px 20px",fontSize:13,fontWeight:700,fontFamily:F.b,boxShadow:"0 4px 20px #0003",maxWidth:340,textAlign:"center",lineHeight:1.5}}>{msg}</div>;
}
...
export default function App(){
  ...
  // campRef removed — camp is always passed explicitly to fetchListings
  ...
  // ── FETCH LISTINGS ─────────────────────────────────────────────────────────
  // Always pass campArg explicitly. Falls back to localStorage — never state/closure.
  const fetchListings = useCallback(async(campArg)=>{
    ...
    setListings((data||[]).map(l=>{
      const u = l["users!listings_user_id_fkey"] || l.users || {};
      return {
  ...
  // ── REALTIME + POLLING ─────────────────────────────────────────────────────
  useEffect(()=>{
    // Read camp from localStorage — guaranteed to be correct even on first render
    const c = localStorage.getItem("sk_camp") || "";
    if(!c) return;

    fetchListings(c);

    const channelName = `listings_${c.replace(/[^a-zA-Z0-9]/g,"_")}`;
    const ch = supabase.channel(channelName)
      .on("postgres_changes",{event:"*",schema:"public",table:"listings"},()=>fetchListings(c))
      .on("postgres_changes",{event:"*",schema:"public",table:"listing_items"},()=>fetchListings(c))
      .subscribe((status)=>{
        if(status==="CHANNEL_ERROR") console.warn("Realtime failed, polling will cover it");
      });

    // Poll every 10s as reliable fallback
    const poll = setInterval(()=>fetchListings(c), 10000);

    return()=>{ supabase.removeChannel(ch); clearInterval(poll); };
  },[camp]); // re-run if camp changes (e.g. user edits profile)

  function showToast(msg){ setToast(msg); setTimeout(()=>setToast(null),3500); }

  async function manualRefresh(){
    setRefreshing(true);
    await fetchListings(localStorage.getItem("sk_camp")||camp);
    setRefreshing(false);
  ...
  async function handleSignup(){
    ...
    try{
      ...
      if(existing){
        setUid(existing.id);setUserName(existing.name);setCamp(existing.camp);setPlatoon(existing.platoon);setPhone(existing.phone||phone);
  ...
  async function requestNotifications(){
    ...
    if(perm==="granted") new Notification("SwapKit 🎯",{body:"You'll be notified when your size is posted!",icon:"/icons/icon-192.png"});
  }
    ...
    try{
      ...
      if(postFile){
        try{
          ...
          else console.warn("Image upload skipped:",upErr.message);
      ...
      setTimeout(()=>fetchListings(localStorage.getItem("sk_camp")||camp),1000);
    }finally{setLoading(false);}
  }
  ...
  // ── MARK DONE — also deletes image from storage ────────────────────────────
  async function handleDone(listing){
    if(listing.img){
      try{
        const path=listing.img.split("/listing-images/")[1];
  ...
  const filtered=listings.filter(l=>{
    ...
    const sm=!search||l.items.some(i=>i.item.toLowerCase().includes(search.toLowerCase())||i.has.toLowerCase().includes(search.toLowerCase())||i.wants.toLowerCase().includes(search.toLowerCase()));
    return im&&sm;
  });
  ...
  if(screen==="onboard")return(
    <div style={WRAP}>
      ...
      <div style={{flex:1,padding:"28px 24px 32px",overflowY:"auto"}}>
        <div style={{marginBottom:16}}><label style={LBL}>Name</label><input style={INP} placeholder="Amaka Adamu" value={userName} onChange={e=>setUserName(e.target.value)}/></div>
        <div style={{marginBottom:16}}>
          <label style={LBL}>Phone number</label>
          <input type="tel" maxLength={11} placeholder="08012345678" value={phone} onChange={e=>setPhone(e.target.value.replace(/\D/g,"").slice(0,11))} style={INP}/>
    ...
    <div style={WRAP}>
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",padding:"24px 22px",overflowY:"auto"}}>
        <div style={{display:"flex",flexDirection:"column",alignItems:"center",marginBottom:24,textAlign:"center"}}>
          ...
          <div style={{fontSize:13,color:C.inkLight,lineHeight:1.7,fontFamily:F.b}}>Reach out to {selected.name.split(" ")[0]} to arrange the swap in camp.</div>
        </div>
        <div style={{background:C.goldFaint,border:`0.5px solid ${C.goldBorder}`,borderRadius:13,padding:"11px 14px",fontSize:12,color:C.goldDark,fontFamily:F.b,fontWeight:600,marginBottom:16,display:"flex",gap:8,alignItems:"center"}}>
          <span style={{fontSize:16}}>👁️</span>This listing is now <strong>Pending</strong>. Auto-clears in 24hrs if not marked done.
        </div>
        <div style={{background:C.white,border:`0.5px solid ${C.border}`,borderRadius:20,padding:18,marginBottom:14}}>
          ...
          {selected.items.map((it,i)=>(
            <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:i<selected.items.length-1?10:0,paddingBottom:i<selected.items.length-1?10:0,borderBottom:i<selected.items.length-1?`0.5px solid ${C.border}`:"none"}}>
              ...
              <div style={{display:"flex",gap:5,alignItems:"center"}}><Tag variant="olive">{it.has}</Tag><span style={{color:C.inkLight,fontSize:12}}>→</span><Tag variant="gold">{it.wants}</Tag></div>
            </div>
          ))}
        </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:16}}>
          <a href={`tel:${selected.phone}`} style={{padding:"14px",borderRadius:14,border:`0.5px solid ${C.border}`,background:C.white,fontSize:13,fontWeight:700,color:C.inkMid,textDecoration:"none",display:"flex",alignItems:"center",justifyContent:"center",gap:6}}>📞 Call</a>
          <a href={`https://wa.me/234${selected.phone?.slice(1)}?text=${encodeURIComponent(`Hi ${selected.name.split(" ")[0]}! 👋 I saw your SwapKit listing and I think we can swap. I'm in ${platoon}. Let's meet up in camp 🤝`)}`} target="_blank" rel="noreferrer" style={{padding:"14px",borderRadius:14,border:"none",background:"#25D366",fontSize:13,fontWeight:700,color:"#fff",textDecoration:"none",display:"flex",alignItems:"center",justifyContent:"center",gap:6}}>💬 WhatsApp</a>
        </div>
    ...
    if(tab==="feed")return(
      <div style={WRAP}>
        ...
        <div style={{background:C.white,borderBottom:`0.5px solid ${C.border}`,padding:"14px 18px 12px",flexShrink:0}}>
          ...
          <div style={{display:"flex",gap:7,overflowX:"auto",paddingBottom:4,scrollbarWidth:"none"}} className="hide-scroll">
        ...
        <div style={{flex:1,padding:"14px 16px 160px",overflowY:"auto"}}>
          ...
          {filtered.length===0?(
            <div style={{textAlign:"center",padding:"56px 24px",color:C.inkLight}}>
              ...
              <div style={{fontSize:13,fontFamily:F.b}}>Be the first to post a swap in camp.</div>
            </div>
          ):filtered.map((l,idx)=>(
            <div key={l.id}>
          ...
          <div style={{flex:1,overflowY:"auto",paddingBottom:80}}>
            ...
            <div style={{padding:"20px 18px"}}>
              <div style={{background:C.white,border:`0.5px solid ${C.border}`,borderRadius:18,padding:18,marginBottom:12}}>
                ...
                <div style={{fontSize:13,color:C.inkMid,lineHeight:1.75,fontFamily:F.b}}>Every year, 350,000+ Nigerian graduates enter NYSC camp and receive khakis and boots — almost none in the right size. SwapKit fixes that. Post what you have. Find what you need. Swap in minutes.</div>
              </div>
              <InlineAd/>
              <div style={{background:C.goldFaint,border:`0.5px solid ${C.goldBorder}`,borderRadius:14,padding:"13px 15px",display:"flex",gap:10,alignItems:"flex-start"}}>
                ...
                <div><div style={{fontWeight:700,fontSize:13,color:C.goldDark,fontFamily:F.b,marginBottom:3}}>Get in touch</div><div style={{fontSize:12,color:C.goldDark,fontFamily:F.b,lineHeight:1.6}}>Advertise or feedback — <a href="mailto:subeyeapp@gmail.com" style={{color:C.goldDark}}>subeyeapp@gmail.com</a></div></div>
              </div>
            </div>
          </div>
      ...
      if(pView==="editprofile")return(
        <div style={WRAP}>
          ...
          <div style={{flex:1,padding:"24px 20px 100px",overflowY:"auto"}}>
            <div style={{marginBottom:16}}><label style={LBL}>Name</label><input style={INP} value={editName} onChange={e=>setEditName(e.target.value)} placeholder={userName}/></div>
            <div style={{marginBottom:16}}>
              <label style={LBL}>Phone number</label>
              ...
              {editPhone.length>0&&editPhone.length<11&&<div style={{fontSize:11,color:C.gold,marginTop:5,fontFamily:F.b}}>{11-editPhone.length} more digits needed</div>}
              <div style={{fontSize:11,color:C.inkLight,marginTop:5,fontFamily:F.b}}>Only shown to corpers who request your swap.</div>
            </div>
            <div style={{marginBottom:16}}>
              ...
              <div style={{fontSize:11,color:C.gold,marginTop:5,fontFamily:F.b}}>⚠️ Changing camp reloads your feed</div>
            </div>
      ...
      if(pView==="settings")return(
        <div style={WRAP}>
          ...
          <div style={{flex:1,overflowY:"auto",paddingBottom:80}}>
            ...
            <div style={{background:C.white,border:`0.5px solid ${C.border}`,borderRadius:16,margin:"8px 16px 14px",overflow:"hidden"}}>
              <SettingRow emoji="🔔" label="Swap match alerts" sub={notifGranted?"On":"Get notified when your size is posted"} right={notifGranted?<Tag variant="done">On</Tag>:<button onClick={requestNotifications} style={{padding:"6px 12px",borderRadius:8,border:"none",background:C.olive,color:C.white,fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:F.b}}>Enable</button>}/>
            </div>
            ...
            <div style={{margin:"0 16px 24px"}}>
              {!confirmDelete
                ...
                :<div style={{background:C.redFaint,border:`0.5px solid ${C.red}44`,borderRadius:14,padding:"16px"}}>
                  ...
                  <div style={{fontSize:12,color:C.red,fontFamily:F.b,marginBottom:14,lineHeight:1.5}}>This permanently deletes your account and all listings.</div>
    ...
    <div style={WRAP}>
      ...
      <div style={{flex:1,padding:"22px 18px 160px",overflowY:"auto"}}>
        ...
        {step===3&&<>
          ...
          <div style={{fontSize:13,color:C.inkLight,marginBottom:16,lineHeight:1.55,fontFamily:F.b}}>Optional{postItems.length>1?` — lay all ${postItems.length} items side by side`:""}. Helps others trust your listing.</div>
